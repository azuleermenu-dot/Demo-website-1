...
function applySettings() {
  ...
  radiusRange.value = settings.radius; radiusLabel.textContent = `${settings.radius}px`; motionToggle.classList.toggle('active', settings.motion === 'on'); motionToggle.setAttribute('aria-pressed', String(settings.motion === 'on'));
  layoutLabel.textContent = ({ split:'Split', center:'Centered', editorial:'Editorial' })[settings.layout] || 'Split';
...
function openLab() { styleLab.removeAttribute('hidden'); styleLab.classList.add('open'); styleLab.setAttribute('aria-hidden', 'false'); closeStyleLab.focus(); body.classList.add('lab-open'); }
function closeLab() { styleLab.classList.remove('open'); styleLab.setAttribute('aria-hidden', 'true'); styleLab.setAttribute('hidden', ''); openStyleLab.focus(); body.classList.remove('lab-open'); }
...
window.addEventListener('keydown', (event) => { if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'k') { event.preventDefault(); styleLab.hasAttribute('hidden') ? openLab() : closeLab(); } if (event.key === 'Escape' && !styleLab.hasAttribute('hidden')) closeLab(); });
