...
function injectAppetitePreferences() {
  ...
  themeGrid.innerHTML = appetiteStyles.map((style) => `<button class="theme-option appetite-option" data-theme-choice="${style.id}" type="button"><span class="swatch ${style.id}-swatch"></span><b>${style.name}</b><small>${style.note}</small></button>`).join('');
...
window.addEventListener('keydown', (event) => { if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'k') { event.preventDefault(); styleLab.classList.contains('open') ? closeLab() : openLab(); } if (event.key === 'Escape' && styleLab.classList.contains('open')) closeLab(); });
...
injectAppetiteStyles(); injectAppetitePreferences(); applySettings();
